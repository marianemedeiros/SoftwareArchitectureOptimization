...
public class GettingStartedWithUML2 {
	...
	protected static Interface createInterface(
		...
		Interface interface_ = package_.createOwnedInterface(name);

		out("Interface '%s' created.", interface_.getQualifiedName());
	...
	protected static Component createComponent(
		...
		Component component_ = UMLFactory.eINSTANCE.createComponent();
		component_.setName(name);
		package_.getPackagedElements().add(component_);
		
		out("Component '%s' created.", component_.getQualifiedName());

		return component_;
	}
	
	protected static Realization createRealization(
			org.eclipse.uml2.uml.Package package_, String name, Interface interface_, Class class_) {

		Realization realization_ = UMLFactory.eINSTANCE.createRealization();
